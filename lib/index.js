...
function innerRpc(request, callback) {
    ...
    var errorMessage = 'Cazcoin JSON-RPC: ';
    ...
    var req = this.protocol.request(options, function (res) {
        ...
        res.on('end', function () {
            ...
            if (res.statusCode === 500 && buf.toString('utf8') === 'Work queue depth exceeded') {
                var exceededError = new Error('Cazcoin JSON-RPC: ' + buf.toString('utf8'));
...
// For definitions of RPC calls, see various files in: https://github.com/copicogithub1/cazcoin/tree/master/src
RpcClient.callspec = {
